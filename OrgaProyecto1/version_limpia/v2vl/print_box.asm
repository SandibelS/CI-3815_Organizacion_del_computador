...
.macro print_box
	# Suposiciones:
	#	$s4 nos idica si tenemos que imprimir una caja de tiempo libre (1 si hay que hacerlo, 0 de lo contrario)
	#	Si $s4 es 1
	#		$a1 un  numero del 0 al 5 que representa el dia en que se estaba buscando la clase
	#		$a2 la dirreccion en memoria en donde comienza el horario que se buscaba
	#	Si $s4 es 0
	#	Se imprime el nodo en $s7
...
# una clase con el mismo horario
	...
	la $a0, salto
	jal print_string			# imprimimos un salto de linea (Para separarnos de la caja de arriba si hay)
	...
	move $s1, $v1				# guardamos la direccion del string dia
	...
	li   $a0, 32
	move $a1, $s3
	jal print_chain				# imprimimos la cantidad de espacios calculadas
