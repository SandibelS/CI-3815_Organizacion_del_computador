...
generate_nodes_loop: beqz $s3, end_generate_nodes
	...
	add  $s2, $s2, 5			# Le sumamos 5, porque vamos a comenzar a buscar la siguiente clase en el dia despues
...
add_node: 
	# Recibe:
	# 	a1: cabeza de la lista a agregar el nodo
	#	a2: direccion del nodo a agregar
	...
	beqz $t0, agregar_primer_elemento	
	...
	move $s0, $a2				# No podemos perder la direccion de nuestro nodo, lo nececitamos para agregarlo
		 				# Se le pasa a s2 la direccion de que hay en la cabeza (que sabemos que no es 0)
	move $s2, $a1
	add  $s2, $a1, 28
