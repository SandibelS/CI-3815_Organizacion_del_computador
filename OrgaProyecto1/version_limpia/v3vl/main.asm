...
.text
	...
	longest_name
	add $s5, $s5, 20 		# A partir de la cantidad de caracateres del nombre mas largo calculamos el tamaño de la caja 
...
input:
	...
	beq $v0, 119, previous_class	# w arriba
	beq $v0, 115, next_class	# s abajo
	beq $v0, 97,  st_previous_day	# a izquierda
	beq $v0, 100, st_next_day	# d derecha
	b   input			# Si no se ingreso una de las teclas especificadas volvemos a preguntar


# Arriba #
previous_class:

	beqz $s4, clc_previous_class	# Si s4 es cero, significa que no se estaba imprimiendo una caja de tiempo libre
	li   $s4, 0			# De lo contrario, ya en $s7 estara el nodo anterior y tendremos que cambiar a s4
	pop ($a1)			# Sacamos del stack el horario y el dia que habiamos guardado
	pop ($a1)
	b print
...
clc_previous_class: 
	...
	bne $t0, $t1, end_previous_class	# Revisamos que la direccion del nodo anterior no apunte a la cabeza
	li $t0, 0
...
clc_next_class:
	...
	add  $s6, $s6, 1	# Como debemos ,movernos al siguiente dia sumamos en uno y luego le aplicamos mod con 6
	li   $t1, 6		# al resultado para obtener el dia en el rango [0,5]
	div  $s6, $t1
	mfhi $s6		
...
st_previous_day:
	...
	st_previous_day_loop: beqz $t2, end_st_previous_day_loop	# Procedmos a buscar el ultimo nodo
		move $t1, $t2
		lw   $t2, 28($t1)
	b st_previous_day_loop
		
	end_st_previous_day_loop:
	
	li   $s4, 1						# Guardamos la señal que nos dice que se imprime una caja de tiempo libre
	move $a1, $t0						# Especificamos en que dia estabamos buscando
...
st_next_day:
	...
	move $t0, $s6				# Vamos a guardar lo que hay en $s6 en t0
	lw   $s0, 8($s7)

	# Si s4 es 1, entonces estamos posicionados en una caja de tiempo libre y en el stack estara el horario a buscar y el dia en
	# que estabamos
	beqz $s4, continue_st_next_day
	lw   $s0 ($sp)			 	# Obtenemos el horario
	lw   $t0 -4($sp)			# Obtenemos el dia en que estabamos  (con la caja libre)
	
continue_st_next_day:
	add  $t0, $t0, 1 		# Primero vamos a revisar que la lista no este vacia, para eso calculamos el numero del dia sig
	li   $t1, 6		
	div  $t0, $t1
	mfhi $t0
	...
	bnez $s4,  st_next_day_loop		
	push ($t0)
	push ($s0)
	
 st_next_day_loop: beqz $t2, end_st_next_day_loop	# Procedemos a buscar el ultimo nodo
		move $t1, $t2
		lw   $t2, 28($t1)	
	b st_next_day_loop
	...
	li   $s4, 1						# Guardamos la señal que nos dice que se imprime una caja de tiempo libre
	move $a1, $t0						# Especificamos en que dia estabamos buscando
	move $a2, $s0						# Cargamos el horario que habiamos buscado
	move $s7, $t1 						# Actualizamos nuestro nodo actual
	b print
...
src_sm_hor:
	...
	move $a2, $s3					# Le pasamos a $a2 el horario que estabamos buscando
...
# Funciones auxiliares para moverse por el horario #
